...
- `offline_browser/snaps/` 放 Chromium 离线 snap 包。
- `DeepStream-Yolo/` 需要包含模型、配置、解析库等运行文件。
- 不要把 `.git/`、`release/`、日志、CSV、运行输出打进正式包。

离线 Chromium 当前按下面这些文件名查找：

```text
offline_browser/snaps/bare_5.snap
offline_browser/snaps/core22_2412.snap
offline_browser/snaps/core24_1588.snap
offline_browser/snaps/cups_1171.snap
offline_browser/snaps/gnome-46-2404_147.snap
offline_browser/snaps/gtk-common-themes_1535.snap
offline_browser/snaps/mesa-2404_1166.snap
offline_browser/snaps/chromium_3318.snap
```
...
- 如果 `offline_browser/snaps/` 已准备好，则离线安装 Chromium。
...
- 自动创建统一日志目录 `~/Project/box_config_tool/logs`。
...
```bash
bash Box_admin/open_box_admin.sh
```
...
如果智能控制平台要求网页登录态，当前配置工具会先用固定账号登录：
...
```bash
~/Project/box_config_tool/release/diagnostics_时间戳.tar.gz
```

压缩包里会包含：

- 三个项目服务的 `systemctl status`
- 三个项目服务的 `journalctl`
- `logs/` 下的运行日志
- `Traffic_detect/log`
- `Traffic_detect/queue_csv`
- 当前配置文件和 DeepStream 配置

## 12. 交付前检查清单

- `Miniforge3-Linux-aarch64.sh` 已放在工程根目录。
- `python_wheels/` 已放在工程根目录。
- `offline_browser/snaps/` 已放入 Chromium 离线 snap 包。
